...
def get_usage_key(device_id):
    return f"{device_id}_{get_today()}"

def can_send(device_id):
    key = get_usage_key(device_id)
...
def send_sms_api():
    ...
    # Create tasks with rotation
    tasks = []
    idx = 0
    while len(tasks) < max_count and idx < len(available_devices):
        device = available_devices[idx % len(available_devices)]
        remaining = get_remaining(device['id'])
        if remaining > 0:
            tasks.append({
    ...
    # Send SMS with threads
    def worker(task):
        success = send_sms(task['config'], task['client_id'], task['number'], task['message'])
        return success, task['client_id'][:8]
    
    with ThreadPoolExecutor(max_workers=10) as ex:
        futures = [ex.submit(worker, task) for task in tasks]
        for future in as_completed(futures):
            success, device_id = future.result()
            if success:
                sent += 1
            else:
                failed += 1
            if device_id not in device_stats:
                device_stats[device_id] = {"sent": 0, "failed": 0}
            if success:
                device_stats[device_id]["sent"] += 1
            else:
...
def status_api():
    ...
    return jsonify({
        ...
        "total_devices": len(all_devices),
        "online_devices": len(online_devices),
        "offline_devices": len(all_devices) - len(online_devices),
        "available_devices": len(available_devices),
        "limit_per_device": MAX_SMS_PER_DEVICE,
        "firebases": len(FIREBASE_CONFIGS),
...
@app.route('/reset')
def reset_api():
    """Reset device usage (for testing)"""
    global device_usage
    device_usage = {}
    return jsonify({
        "success": True,
